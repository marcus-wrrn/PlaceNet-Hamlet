...
pub(super) fn spawn_tls_accept_loop(mut gateway: BoundGateway, tls_acceptor: TlsAcceptor) {
    tokio::spawn(async move {
        ...
        loop {
            ...
            tokio::spawn(super::internals::proxy::serve_tls_connection(
                tcp_stream,
                tls_acceptor.clone(),
                gateway.state.clone(),
                peer_addr,
...
pub(super) fn spawn_plain_accept_loop(mut gateway: BoundGateway) {
    tokio::spawn(async move {
        info!(
        ...
        loop {
            ...
            tokio::spawn(super::internals::proxy::serve_connection(tcp_stream, gateway.state.clone(), peer_addr));
        }
    });
}
